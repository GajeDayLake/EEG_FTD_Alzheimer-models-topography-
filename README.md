...
**eeg_ftd_alzheimer_2.py** : Shares the same data pipeline as file 1 (loading, segmentation, CWT scalograms). Trains a pretrained YOLO26-cls backbone adapted for 19-channel EEG input (first conv patched 3→19 ch, classifier head patched →3 classes, forward overridden to return raw logits). <br> <br>  
...
| CWT (cmor-1.0-1.0, 60 scales) | Tensor (19, 60, 1000) — 19-channel scalogram matrices | 20 sec splits, %70 train, %10 val, %20 test | YOLO26n-cls (Ultralytics, modified: first conv 3→19 ch, final linear→3 classes, logits override) | Dropout= 0.15, LR schedule= cosine decay (T_max=120), No early stopping, Epochs= 120, Batch= 8, Learning rate= 0.0001, Weight decay= 0.0001, Optimizer= AdamW, Loss= Weighted Cross Entropy (label smoothing=0.05) | **Test= %61.74** (seg) / **%66.67** (subj), Train= ~%85.10 | **Test= %61.73** (seg) / **%67.29** (subj), Train= ~%85 | **Test= %61.74** (seg) / **%68.52** (subj), Train= ~%85 | **Test= %61.74** (seg) / **%66.67** (subj), Train= ~%85 | ... |
